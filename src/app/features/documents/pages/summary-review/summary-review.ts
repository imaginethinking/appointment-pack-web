...
export class SummaryReview implements OnInit {
  ...
  protected readonly canWriteHistory = computed(() =>
    this.authorisation.can(this.selectedPatientState.selectedPatient(), 'history', 'edit'),
  );

  protected readonly canAccept = computed(() =>
    this.canEditDocument() && this.canWriteHistory(),
  );

  protected readonly canRetry = computed(() =>
    this.status() === 'failed' && this.canEditDocument() && !this.isRetrying(),
  );

  protected readonly canReject = computed(() =>
    this.status() === 'ready' && this.canEditDocument(),
  );
  ...
  protected retrySummarisation(): void {
    ...
    if (approvedText === null || approvedText.length === 0) {
      ...
      this.errorMessage.set(
        'The approved de-identified text required for retry is not available.',
      );
    ...
    this.documentApi
      ...
      .subscribe({
        next: (processingResult) => {
          if (
            processingResult.status !== 'READY_FOR_SUMMARY_REVIEW' ||
            processingResult.generatedSummary === null ||
            processingResult.generatedSummary.trim().length === 0
          ) {
            this.status.set('error');
  ...
  protected accept(): void {
    ...
    this.documentApi
      .acceptDocumentSummary(document.id, {
    ...
    this.documentApi
      ...
      .pipe(
        switchMap((document) => {
          ...
          if (document.documentType !== 'CONSULTATION_OUTCOME_LETTER') {
            this.status.set('invalid');
            this.errorMessage.set(
              'Only consultation outcome letters use summary review.',
            );
          ...
          if (
            ...
            document.status !== 'SUMMARISATION_FAILED'
          ) {
            this.status.set('invalid');
            this.errorMessage.set(
              'This consultation document is not awaiting summary review or summarisation recovery.',
            );
      ...
      .subscribe({
        next: (processing) => {
          ...
          this.status.set('invalid');
          this.errorMessage.set(
            'The document processing state changed while the review was loading.',
          );
        },
        error: (error: unknown) => {
  ...
  private prepareFailedSummary(processing: DocumentProcessingResultResponse): void {
    if (
      processing.approvedDeidentifiedText === null ||
      processing.approvedDeidentifiedText.length === 0
    ) {
      this.status.set('error');
      this.errorMessage.set(
        'The approved de-identified text required for recovery is not available.',
      );
  ...
  private handleLoadError(error: unknown): void {
    ...
    this.status.set('error');
    this.errorMessage.set(
      getHttpErrorMessage(error, 'Unable to load the summary review.'),
    );
  }
  ...
  private handleRetryError(error: unknown): void {
    ...
    if (hasHttpStatus(error, 409)) {
      this.status.set('invalid');
      this.errorMessage.set(
        'The document state changed before summarisation could be retried. Return to document details and reload its current state.',
      );
    ...
    if (hasHttpStatus(error, 413)) {
      this.actionError.set(
        'The approved de-identified text is too large to summarise.',
      );
    ...
    if (hasHttpStatus(error, 502)) {
      this.actionError.set(
        'The external summarisation service returned an invalid response. You can retry again or enter a manual summary.',
      );
    ...
    if (hasHttpStatus(error, 503)) {
      this.actionError.set(
        'External summarisation is currently unavailable. You can retry later or enter a manual summary.',
      );
    ...
    if (hasHttpStatus(error, 504)) {
      this.actionError.set(
        'External summarisation timed out. You can retry again or enter a manual summary.',
      );
    ...
    this.actionError.set(
      getHttpErrorMessage(error, 'Unable to retry summarisation.'),
    );
  }
  ...
  private handleAcceptError(error: unknown): void {
    ...
    this.actionError.set(
      getHttpErrorMessage(error, 'Unable to accept the summary.'),
    );
  }
  ...
  private handleRejectError(error: unknown): void {
    ...
    if (hasHttpStatus(error, 409)) {
      this.status.set('invalid');
      this.errorMessage.set(
        'The document state changed before the summary could be rejected.',
      );
    ...
    this.actionError.set(
      getHttpErrorMessage(error, 'Unable to reject the summary.'),
    );
  }
