...
export class DocumentUpload {
  ...
  protected readonly form = this.formBuilder.group({
    documentType: this.formBuilder.nonNullable.control<DocumentType | ''>(
      '',
      Validators.required,
    ),
  });
  ...
  protected upload(): void {
    ...
    if (
      ...
      this.errorMessage.set(
        'You do not have permission to upload documents for the selected patient.',
      );
      ...
      .subscribe({
        next: (document) => {
          if (
            ...
            this.errorMessage.set(
              'The document was uploaded successfully, but the selected patient changed during the upload.',
            );
  ...
  private validateFile(file: File): string | null {
    ...
    const supported = SUPPORTED_FILE_EXTENSIONS.some(
      (extension) => lowerCaseName.endsWith(extension),
    );

    if (!supported) {
      return 'Select a PDF, JPEG or PNG document.';
    }
  ...
  private handleUploadError(error: unknown): void {
    if (hasHttpStatus(error, 413)) {
      this.errorMessage.set(
        'The selected document exceeds the maximum upload size.',
      );
    ...
    this.patientContextCoordinator.load().subscribe({
      next: () => {
        ...
        if (selectedPatient === null) {
          this.errorMessage.set(
            'Your patient access has changed. Select a patient and try again.',
          );
        ...
        if (!this.canUpload()) {
          this.errorMessage.set(
            'You no longer have permission to upload documents for this patient.',
          );
        ...
        this.errorMessage.set(
          'The patient record is no longer available for this upload. Please try again.',
        );
      },
