...
export class AppointmentCreate {
  ...
  protected create(): void {
    ...
    this.appointmentApi.createAppointment(selectedPatient.patientRecordId, mapAppointmentFormToRequest(this.form)).pipe(
      finalize(() => this.isSaving.set(false)),
    ).subscribe({
      next: (appointment) => void this.router.navigate(['/appointments', appointment.id]),
